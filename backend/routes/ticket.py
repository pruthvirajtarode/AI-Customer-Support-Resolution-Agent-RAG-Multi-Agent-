...
@router.post("/submit")
def submit_ticket(ticket_text: str, order_json: str, db: Session = Depends(SessionLocal), user=Depends(get_current_user)):
...
@router.get("/list")
def list_tickets(db: Session = Depends(SessionLocal), user=Depends(get_current_user)):
    return db.query(Ticket).filter(Ticket.user_id == user.id).all()

@router.get("/responses")
def get_responses(db: Session = Depends(SessionLocal), user=Depends(get_current_user)):
